...
void genera_filtrate(Node *root_ref, Node **filt_ref, List **head, List **tail)
{
    num_filtered = 0;
...
int confronto(Node *root_ref, Node **filt_ref, List **head, List **tail, char *g, char *t, int flag)
{
...
void game(Node **root_ref)
{
    ...
    while (num_guesses > 0)
    {
        ...
        if (search(*root_ref, buffer))
        {
            ...
            if (confronto(*root_ref, &root_filtrate, &head, &tail, buffer, target, flag))
            {
                puts("ok");
                break;
            }
